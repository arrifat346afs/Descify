fn main() {
    ...
    let exiftool_check = std::process::Command::new("exiftool").arg("-ver").output();

    match exiftool_check {
        Ok(output) if output.status.success() => {
            let version = String::from_utf8_lossy(&output.stdout);
            println!(
                "cargo:warning=ExifTool found in system PATH (version {})",
                version.trim()
            );
        }
        _ => {
            ...
            println!(
                "cargo:warning=  3. Make it executable: chmod +x src-tauri/resources/exiftool"
            );
        }
    }
